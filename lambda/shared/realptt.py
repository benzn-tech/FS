...
# RealPTT uses timeZoneOffset = -480 (as confirmed via Postman)
TIMEZONE_OFFSET = -480
REQUEST_TIMEOUT = 30
...
class RealPTTClient:
    ...
    def login(self) -> None:
        ...
        # Formula: HMAC-SHA1(message=sha1(password), key=random)
        # Matches CryptoJS.HmacSHA1(sha1Password, random) where args are (message, key)
    ...
    # ------------------------------------------------------------------
    # Upload file queries (body cam videos, audio, photos)
    # ------------------------------------------------------------------
    ...
    def get_upload_files(
        ...
        """
        Query uploaded files via GET /ptt/uploadFile?method=get

        Args:
            user_account: RealPTT user account to query
            start_date:   "YYYY-MM-DD" — earliest upload date (NZ local)
            end_date:     "YYYY-MM-DD" — latest upload date (NZ local), inclusive
            page:         1-based page number
            limit:        results per page (max 20)

        Returns:
            (files, total_pages) where files is a list of:
              { id, file_name, file_type, down_path,
        ...
        params: dict = {
            ...
            "page":         page,
            "limit":        limit,
            "sort":         "0",   # ASC — oldest first
        }
        if start_date:
            params["start_time"] = start_date
        if end_date:
            params["end_time"] = end_date
        data = self._get("/ptt/uploadFile", params=params)
        files       = data.get("uploadFiles", [])
        total_pages = int(data.get("pageSize", 1))
        return files, total_pages

    def get_all_upload_files_since(self, user_account: str, start_date: str, end_date: str | None = None) -> list[dict]:
        """
        Paginate through all uploaded files for a user since start_date.
        Returns a flat list of all file dicts.
        """
        all_files: list[dict] = []
        page = 1

        while True:
            files, total_pages = self.get_upload_files(
                user_account=user_account,
        ...
        logger.info(
            "Fetched %d total files for %s since %s",
            len(all_files), user_account, start_date,
        )
        return all_files
